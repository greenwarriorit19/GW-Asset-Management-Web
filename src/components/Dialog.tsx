...
export interface DialogOptions {
  ...
  /** Rows shown as a preview of exactly what is about to go. */
  preview?: [string, string][];
  /** The text that has to be typed back before the action is allowed (an asset ID, say). */
...
/** Delete confirmation in the style of AWS: a preview of the record, the exact name typed back, and a reason. */
export const askDeleteWithPhrase = (opts: DialogOptions & { confirmPhrase: string }) =>
  open({ confirmLabel: 'Delete', danger: true, ...opts, reason: opts.reason === false ? false : { minLength: 3, ...(opts.reason || {}) } });
...
export function DialogHost() {
  ...
  useEffect(() => { setText(''); setPhrase(''); if (p) setTimeout(() => (p.opts.confirmPhrase || p.opts.reason ? input.current : btn.current)?.focus(), 0); }, [p]);
  useEffect(() => {
    if (!p) return;
  ...
  const valid = (!opts.reason || text.trim().length >= min) && phraseOk;
    ...
    <div className="modal-backdrop" style={{ zIndex: 200, alignItems: 'center' }} onMouseDown={e => { if (e.target === e.currentTarget) settle(null); }}>
      <form className="modal" style={{ maxWidth: 460 }} role="alertdialog" aria-modal="true" onSubmit={e => { e.preventDefault(); confirm(); }}>
        ...
        <div className="modal-body">
          {opts.message && <p style={{ marginBottom: opts.reason || opts.preview ? 12 : 0 }}>{opts.message}</p>}
          {opts.preview && (
            <table className="data preview" style={{ marginBottom: 12 }}>
              <tbody>{opts.preview.map(([k, v]) => <tr key={k}><td className="k">{k}</td><td>{v}</td></tr>)}</tbody>
            </table>
          )}
          {opts.confirmPhrase && (
            <div className="field" style={{ marginBottom: 12 }}>
              <label>Type <span className="mono">{opts.confirmPhrase}</span> to confirm<span className="req">*</span></label>
              <input ref={input} value={phrase} onChange={e => setPhrase(e.target.value)} placeholder={opts.confirmPhrase} autoComplete="off" spellCheck={false} />
              {phrase.length > 0 && !phraseOk && <span className="hint" style={{ color: 'var(--danger)' }}>That does not match {opts.confirmPhrase}.</span>}
            </div>
          )}
          {opts.reason && (
            <div className="field">
              <label>{opts.reason.label ?? 'Reason'}<span className="req">*</span></label>
              <input ref={opts.confirmPhrase ? undefined : input} value={text} onChange={e => setText(e.target.value)} placeholder={opts.reason.placeholder ?? 'Why is this being done? (recorded in the audit log)'} />
