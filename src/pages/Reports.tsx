...
export const REPORTS: Report[] = [
  ...
  { key: 'returnpending', name: 'Return Pending Report', build: (db, s) => [
    ...
    ...db.returns.filter(r => !r.inspected).map(r => ({ Type: 'Return not completed', Reference: r.id, 'Asset ID': r.assetId, Employee: s.employeeName(r.employeeId), 'Expected Return': r.date, 'Days Overdue': Math.ceil((Date.now() - new Date(r.date).getTime()) / 86400000) })),
  ] },
