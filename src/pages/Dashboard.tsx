  ...
  return (
    <>
      ...
      <div className="grid cols-6" style={{ marginBottom: 18 }}>
        ...
        <div className="metric"><div className="label">Awaiting Action</div><div className="value">{db.transfers.filter(t => ['Awaiting Approval', 'Approved'].includes(t.status)).length + db.returns.filter(r => !r.inspected).length + db.incidents.filter(i => i.status !== 'Closed').length}</div><div className="sub">Transfers, returns, incidents</div></div>
      </div>
